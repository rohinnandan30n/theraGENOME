"""
Secure Redis cache for Dev3 - Drug Safety & Toxicity API.

Security Rules:
1. Key prefix: "dev3:" (prevents cross-service cache poisoning)
...
DEFAULT_PGX_TTL = 86400  # 24 hours for pharmacogenomics (stable data)
DEFAULT_TOXICITY_TTL = 3600  # 1 hour for toxicity lookups


class Dev3RedisCache:
    """
    Secure Redis cache for Dev3 - Drug Safety & Toxicity API.
    
    SECURITY RULES:
    - All keys prefixed with "dev3:" to prevent cross-service poisoning
    ...
    USAGE PATTERNS:
    1. PGx lookup:
       key = "dev3:pgx:{gene_id}:{drug}"
       value = { "metabolism": "poor", "recommendation": "reduce_dose" }
       ttl = 86400
    
    2. Toxicity lookup:
       key = "dev3:toxicity:{drug}:{pathway}"
       value = { "risk_level": "low", "affected_organs": ["liver"] }
       ttl = 3600
    """
    ...
    async def _get_client(self) -> aioredis.Redis:
        ...
        if self._client is None:
            try:
                ...
                logger.info("Dev3 Redis cache connected")
    ...
    async def close(self) -> None:
        ...
        if self._client:
            try:
                ...
                logger.info("Dev3 Redis cache connection closed")
            except Exception as e:
                logger.warning(f"Error closing Redis connection: {str(e)}")


# Singleton instance for this service
_cache_instance: Optional[Dev3RedisCache] = None


async def get_cache() -> Dev3RedisCache:
    """Get or create singleton cache instance."""
    global _cache_instance
    if _cache_instance is None:
        _cache_instance = Dev3RedisCache()
